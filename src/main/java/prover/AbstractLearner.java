    ...
    public List<Expr> learnFromTerm(TermNode term){
        ...
        if(term.getChildren()==null || term.getChildren().size()==0){
            if(this.symbolicArguments.contains(operator)){ // variable
                // symbol 0: substitute var
                // symbol i: the i-th args, i.e., the args[i-1]
                Expr substVar = ctx.mkConst(ctx.mkSymbol(0),this.sortMap.get(operator));
                this.predicateSet.addPredicate(ctx.mkEq(substVar,ctx.mkConst(ctx.mkSymbol(this.symbolicArguments.indexOf(operator)+1),this.sortMap.get(operator))));
                predicateCandidates.add(ctx.mkConst(ctx.mkSymbol(this.symbolicArguments.indexOf(operator)+1),this.sortMap.get(operator)));
            }else{
                Expr ct = ProverUtilities.parseString2Const(ctx, operator);
